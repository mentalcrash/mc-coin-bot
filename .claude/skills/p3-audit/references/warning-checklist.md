...
> FAIL 사유는 아니지만 **반드시 기록**하여 후속 Phase에서 참고.
...
| 10 ~ 20 | OK — 합리적 |
| 5 ~ 10 | WARNING — 과적합 주의 |
| < 5 | WARNING (강) — Phase 5B에서 파라미터 안정성 면밀 검증 필요 |

> 거래 수는 Phase 4A에서 확인되므로, Phase 3 시점에서는 **파라미터 수만 기록**.
...
Phase 3에서는 **코드 구조적 유사성**으로 사전 판단:
...
> 실제 상관 계수는 Phase 4A 이후 백테스트 결과로 정량 검증.

---

## 크립토 시장 특화 추가 경고

Phase 3 필수 항목은 아니지만, 리포트에 해당 사항 기록:
