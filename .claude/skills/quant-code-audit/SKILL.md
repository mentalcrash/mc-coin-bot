...
description: >
  시니어 퀀트 개발자 관점의 암호화폐/주식 트레이딩 코드 정밀 감사.
  전략 로직, 백테스트 엔진, 주문 실행, 리스크 관리, EDA 컴포넌트 코드를 비판적으로 분석하여
  look-ahead bias, 데이터 누수, 과적합, 실행 비현실성, 손절 미작동, 포지션 사이징 오류,
  이벤트 체인 결함 등 실전 배포 전 반드시 잡아야 할 결함을 탐지한다.
...
### 4-EDA: 이벤트 기반 리스크 관리 체크

EDA 시스템 코드가 포함된 경우 추가 검증:

- [ ] **`await bus.flush()` 호출 확인**: DataFeed에서 bar 발행 후 flush 없으면 모든 파생 이벤트가 마지막 bar 가격으로 체결
  ```bash
  # 탐지: DataFeed에서 publish 후 flush 없는 패턴
  grep -n "publish.*BarEvent" src/eda/*.py
  grep -n "await.*flush" src/eda/*.py
  ```
- [ ] **Equity 이중 계산 방지**: `total_equity = cash + notional + unrealized`는 잘못됨 (notional에 이미 unrealized 포함). 정답: `cash + long_notional - short_notional`
  ```bash
  grep -rn "cash.*notional.*unrealized\|unrealized.*notional.*cash" src/eda/
  ```
- [ ] **`_stopped_this_bar` 가드 동작**: stop-loss 발동 후 같은 bar에서 재진입 방지
  ```bash
  grep -n "_stopped_this_bar" src/eda/portfolio_manager.py
  ```
- [ ] **Batch mode에서 `flush_pending_signals()` 호출**: 멀티에셋 모드에서 signal 수집 후 일괄 주문 발행
  ```bash
  grep -n "flush_pending_signals" src/eda/runner.py
  ```
- [ ] **ATR 계산 순서**: `_update_atr()`는 `pos.last_price` 변경 전에 호출해야 함
- [ ] **RM drawdown 순서**: drawdown 먼저 계산 후 peak equity 업데이트
...
### 6-EDA: 이벤트 시스템 코드 품질 체크

- [ ] **NEVER_DROP_EVENTS 분류 정확성**: 주문/체결/밸런스 등 재무 이벤트가 DROPPABLE로 분류되어 있으면 CRITICAL
  ```bash
  grep -A5 "NEVER_DROP_EVENTS" src/core/events.py
  grep -A5 "DROPPABLE_EVENTS" src/core/events.py
  ```
- [ ] **correlation_id 전파 누락**: 파생 이벤트 발행 시 원본의 correlation_id를 전파하지 않으면 이벤트 추적 불가
  ```bash
  grep -n "correlation_id" src/eda/*.py | grep -v "import\|#"
  ```
- [ ] **핸들러 에러 격리 패턴 준수**: 핸들러 예외가 다른 핸들러에 전파되지 않는지 확인
- [ ] **AnyEvent union 완전성**: 새 이벤트 모델이 `AnyEvent`에 등록되어 있는지 (미등록 시 JSONL 감사 로그 직렬화 실패)
- [ ] **EventType ↔ EVENT_TYPE_MAP 동기화**: enum과 map이 일치하는지
