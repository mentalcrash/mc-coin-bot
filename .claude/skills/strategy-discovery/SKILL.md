...
### Step 4.5: 후보 문서 기록 — temp-candidate.md

Gate 0 PASS인 아이디어를 `docs/strategy/temp-candidate.md`에 기록한다.
이 파일은 **임시 후보 목록**으로, 구현 전 사용자 리뷰와 우선순위 결정을 위한 staging 문서다.

#### 파일 규칙

```
경로: docs/strategy/temp-candidate.md
- 기존 내용이 있으면 **아래에 append** (덮어쓰기 금지)
- 세션 날짜별로 구분 (## 날짜 헤더)
- Gate 0 PASS 아이디어만 기록 (FAIL/WATCH는 기록하지 않음)
- 구현 완료 후 해당 항목을 "✅ 구현 완료" 또는 "❌ 폐기"로 상태 갱신
- 사용자가 구현을 승인하면 scorecard로 이관
```

#### 기록 포맷

```markdown
## {날짜} — Strategy Discovery Session

### 후보 #{번호}: {전략 이름} (`{registry-name}`)

| 항목 | 내용 |
|------|------|
| **카테고리** | Microstructure / Vol / Carry / Behavioral / Info Theory / ... |
| **타임프레임** | 1D / 4H / 1H |
...
──────────────────────────────────────────────────────
  1순위: [아이디어 #X] → 구현 진행
  근거: [선택 이유]
  📄 후보 기록: docs/strategy/temp-candidate.md
══════════════════════════════════════════════════════
```

### 2. 후보 문서 기록 (자동)

Gate 0 PASS 아이디어는 **자동으로** `docs/strategy/temp-candidate.md`에 기록한다.
