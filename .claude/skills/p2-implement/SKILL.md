...
| 데이터 요구사항 | O | OHLCV only / Derivatives (funding_rate, open_interest 등) |
...
## Step 2.5: 레짐 적응형 전략 구현 가이드 (해당 시)

발굴 단계에서 "레짐 활용 여부"가 지정된 경우, 아래 두 가지 접근 중 하나를 선택한다.
...
## Step 2.6: Derivatives 데이터 전략 구현 가이드 (해당 시)
...
class TestRegimeAdaptation:  # 레짐 적응형 전략에만 추가
    ...
    def test_trending_aggressive(self):       # trending에서 더 공격적 (높은 strength)
    def test_volatile_conservative(self):     # volatile에서 보수적 (낮은 strength)
...
| 11 | 레짐 fallback (해당 시) | regime 컬럼 없이도 작동 확인 |
