#!/usr/bin/env python3
"""Gate 1: 6개 전략 (carry-cond-mom, cgo-mom, jump-drift-mom, kelt-eff-trend,
vardecomp-mom, vov-mom) x 5-coin 백테스트.

Usage:
    uv run python scripts/gate1_batch6.py
"""
...
# strategy_name -> (timeframe, needs_derivatives)
STRATEGIES: dict[str, tuple[str, bool]] = {
    "carry-cond-mom": ("4H", True),
    "cgo-mom": ("1D", False),
    "jump-drift-mom": ("4H", False),
    "kelt-eff-trend": ("4H", False),
    "vardecomp-mom": ("1D", False),
    "vov-mom": ("1D", False),
}
...
def main() -> None:
    ...
    for strategy_name, (tf, needs_deriv) in STRATEGIES.items():
        ...
        for symbol in SYMBOLS:
            ...
            entry = run_single(engine, service, strategy_name, symbol, tf, needs_deriv)
    ...
    # Save
    output_path = RESULTS_DIR / "gate1_batch6_results.json"
    output = {
        "meta": {
            ...
            "symbols": SYMBOLS,
            "strategies": {k: {"tf": v[0], "needs_deriv": v[1]} for k, v in STRATEGIES.items()},
    ...
    summary_table = Table(title="Batch6 Gate 1 Results")
    ...
    for strategy_name, results in all_results.items():
        if not results:
            summary_table.add_row(
                strategy_name, STRATEGIES[strategy_name][0],
                "-", "-", "-", "-", "-", "[red]ERROR[/]"
            )
            continue
