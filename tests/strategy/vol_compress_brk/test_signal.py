"""Tests for vol-compress-brk signal generation."""
...
@pytest.fixture
def config() -> VolCompressBrkConfig:
    return VolCompressBrkConfig()
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: VolCompressBrkConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: VolCompressBrkConfig
    ) -> None:
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: VolCompressBrkConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
