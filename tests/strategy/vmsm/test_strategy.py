"""Tests for vmsm strategy integration."""
...
class TestRegistry:
    def test_registered(self) -> None:
        assert "vmsm" in list_strategies()

    def test_get_strategy(self) -> None:
        cls = get_strategy("vmsm")
        assert cls is VmsmStrategy


class TestVmsmStrategy:
    def test_name(self) -> None:
        strategy = VmsmStrategy()
        assert strategy.name == "vmsm"
    ...
    def test_config(self) -> None:
        strategy = VmsmStrategy()
        assert isinstance(strategy.config, VmsmConfig)
    ...
    def test_from_params(self) -> None:
        strategy = VmsmStrategy.from_params(roc_short=6)
        assert isinstance(strategy, VmsmStrategy)

    def test_recommended_config(self) -> None:
        config = VmsmStrategy.recommended_config()
    ...
    def test_custom_config(self) -> None:
        ...
        strategy = VmsmStrategy(config=config)
        assert strategy._config.roc_short == 6
