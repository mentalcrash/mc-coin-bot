"""Tests for Keltner Efficiency Trend signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: KeltEffTrendConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(self, preprocessed_df: pd.DataFrame, config: KeltEffTrendConfig) -> None:
    ...
    def test_entries_exits_bool(
        self, preprocessed_df: pd.DataFrame, config: KeltEffTrendConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: KeltEffTrendConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
...
class TestKeltnerLogic:
    def test_lower_er_threshold_more_signals(self, sample_ohlcv_df: pd.DataFrame) -> None:
        """ER threshold 낮으면 더 많은 시그널 발생."""
        config_strict = KeltEffTrendConfig(er_threshold=0.9)
        config_loose = KeltEffTrendConfig(er_threshold=0.0)
