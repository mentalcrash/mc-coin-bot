"""Tests for MAD-Channel Multi-Scale Trend signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: MadChannelTrendConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: MadChannelTrendConfig
    ) -> None:
...
class TestShortMode:
    ...
    def test_hedge_only_shorts_require_drawdown(self, sample_ohlcv_df: pd.DataFrame) -> None:
        ...
        config = MadChannelTrendConfig(short_mode=ShortMode.HEDGE_ONLY)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
...
class TestConsensusLogic:
    def test_unanimous_long(self, config: MadChannelTrendConfig) -> None:
    ...
    def test_entry_threshold_filters_weak_consensus(self, sample_ohlcv_df: pd.DataFrame) -> None:
        ...
        config_low = MadChannelTrendConfig(entry_threshold=0.0)
        config_high = MadChannelTrendConfig(entry_threshold=0.67)
    ...
    def test_nine_channels_consensus(self, config: MadChannelTrendConfig) -> None:
