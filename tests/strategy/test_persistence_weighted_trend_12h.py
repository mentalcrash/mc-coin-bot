"""Persistence-Weighted-Trend 12H 전략 테스트."""
...
@pytest.fixture()
def config() -> PersistenceWeightedTrendConfig:
    return PersistenceWeightedTrendConfig(
        scale_short=10,
        scale_mid=20,
        scale_long=40,
        mom_lookback=10,
...
class TestConfig:
    ...
    def test_scale_order_validation(self) -> None:
        with pytest.raises(ValueError, match="scale_short < scale_mid < scale_long"):
            PersistenceWeightedTrendConfig(scale_short=30, scale_mid=20, scale_long=60)
...
class TestPreprocessor:
    ...
    def test_preprocess_missing_columns(self, config: PersistenceWeightedTrendConfig) -> None:
    ...
    def test_persistence_range(
        ...
        """Persistence score는 0~1 범위."""
        result = preprocess(ohlcv_df, config)
        for s in (10, 20, 40):
            col = f"persistence_{s}"
            valid = result[col].dropna()
            if len(valid) > 0:
                assert (valid >= 0).all(), f"{col} has negative values"
                assert (valid <= 1).all(), f"{col} exceeds 1"
...
class TestSignal:
    ...
    def test_entries_exits_exclusive(
        self,
        ohlcv_df: pd.DataFrame,
        config: PersistenceWeightedTrendConfig,
    ) -> None:
    ...
    def test_high_threshold_fewer_entries(self, ohlcv_df: pd.DataFrame) -> None:
        """높은 persistence_threshold → 진입 감소."""
        cfg_low = PersistenceWeightedTrendConfig(
            scale_short=10,
            scale_mid=20,
            scale_long=40,
            mom_lookback=10,
        ...
        cfg_high = PersistenceWeightedTrendConfig(
            scale_short=10,
            scale_mid=20,
            scale_long=40,
            mom_lookback=10,
        ...
        p_low = preprocess(ohlcv_df, cfg_low)
        s_low = generate_signals(p_low, cfg_low)
        p_high = preprocess(ohlcv_df, cfg_high)
        s_high = generate_signals(p_high, cfg_high)

        entries_low = s_low.entries.sum()
        entries_high = s_high.entries.sum()
        assert entries_high <= entries_low
...
class TestStrategy:
    def test_registry(self) -> None:
        ...
        cls = get_strategy("persistence-weighted-trend-12h")
        assert cls is PersistenceWeightedTrend12hStrategy

    def test_run(self, ohlcv_df: pd.DataFrame) -> None:
        strategy = PersistenceWeightedTrend12hStrategy(
            config=PersistenceWeightedTrendConfig(
                scale_short=10,
                scale_mid=20,
                scale_long=40,
    ...
    def test_from_params(self) -> None:
        strategy = PersistenceWeightedTrend12hStrategy.from_params(
            scale_short=10,
            scale_mid=20,
            scale_long=40,
        )
        assert strategy.name == "persistence-weighted-trend-12h"

    def test_recommended_config(self) -> None:
        cfg = PersistenceWeightedTrend12hStrategy.recommended_config()
    ...
    def test_startup_info(self) -> None:
        ...
        assert "persistence_threshold" in info
