"""Tests for Disposition CGO preprocessor."""

from __future__ import annotations

import numpy as np
import pandas as pd
import pytest

from src.strategy.disposition_cgo.config import DispositionCgoConfig
from src.strategy.disposition_cgo.preprocessor import preprocess


@pytest.fixture
def config() -> DispositionCgoConfig:
    return DispositionCgoConfig()


@pytest.fixture
def sample_ohlcv_df() -> pd.DataFrame:
    np.random.seed(42)
    n = 300
    close = 100 + np.cumsum(np.random.randn(n) * 2)
    high = close + np.abs(np.random.randn(n) * 1.5)
    low = close - np.abs(np.random.randn(n) * 1.5)
    open_ = close + np.random.randn(n) * 0.5
    volume = np.random.randint(1000, 10000, n).astype(float)
    high = np.maximum(high, np.maximum(open_, close))
    low = np.minimum(low, np.minimum(open_, close))
    return pd.DataFrame(
        {"open": open_, "high": high, "low": low, "close": close, "volume": volume},
        index=pd.date_range("2024-01-01", periods=n, freq="1D"),
    )


class TestPreprocess:
    def test_output_columns(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        result = preprocess(sample_ohlcv_df, config)
        required = {
            "returns",
            "realized_vol",
            "vol_scalar",
            "reference_price",
            "cgo",
            "cgo_smooth",
            "momentum",
            "overhang_spread",
            "overhang_spread_ma",
            "drawdown",
        }
        assert required.issubset(set(result.columns))

    def test_same_length(self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig) -> None:
        result = preprocess(sample_ohlcv_df, config)
        assert len(result) == len(sample_ohlcv_df)

    def test_missing_columns(self, config: DispositionCgoConfig) -> None:
        df = pd.DataFrame({"close": [1, 2, 3]})
        with pytest.raises(ValueError, match="Missing required columns"):
            preprocess(df, config)

    def test_vol_scalar_positive(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        result = preprocess(sample_ohlcv_df, config)
        valid = result["vol_scalar"].dropna()
        assert (valid > 0).all()

    def test_reference_price_positive(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        result = preprocess(sample_ohlcv_df, config)
        valid = result["reference_price"].dropna()
        assert (valid > 0).all()

    def test_cgo_bounded(self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig) -> None:
        """CGO is typically bounded in reasonable range."""
        result = preprocess(sample_ohlcv_df, config)
        valid = result["cgo"].dropna()
        # CGO should be finite
        assert np.isfinite(valid).all()

    def test_overhang_spread_finite(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        result = preprocess(sample_ohlcv_df, config)
        valid = result["overhang_spread"].dropna()
        assert np.isfinite(valid).all()

    def test_drawdown_non_positive(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        result = preprocess(sample_ohlcv_df, config)
        valid = result["drawdown"].dropna()
        assert (valid <= 0).all()

    def test_custom_config(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = DispositionCgoConfig(turnover_window=30, cgo_smooth_window=5)
        result = preprocess(sample_ohlcv_df, config)
        assert "cgo_smooth" in result.columns


class TestPreprocessorImmutability:
    def test_original_unchanged(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        original = sample_ohlcv_df.copy()
        preprocess(sample_ohlcv_df, config)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)

    def test_output_is_new_object(
        self, sample_ohlcv_df: pd.DataFrame, config: DispositionCgoConfig
    ) -> None:
        result = preprocess(sample_ohlcv_df, config)
        assert result is not sample_ohlcv_df
