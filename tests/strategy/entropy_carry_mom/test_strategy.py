"""Tests for Entropy-Carry-Momentum strategy integration."""

from __future__ import annotations

import numpy as np
import pandas as pd
import pytest

from src.strategy import get_strategy, list_strategies
from src.strategy.entropy_carry_mom.config import EntropyCarryMomConfig
from src.strategy.entropy_carry_mom.strategy import EntropyCarryMomStrategy


@pytest.fixture
def sample_df() -> pd.DataFrame:
    np.random.seed(42)
    n = 300
    close = 100 + np.cumsum(np.random.randn(n) * 2)
    high = close + np.abs(np.random.randn(n) * 1.5)
    low = close - np.abs(np.random.randn(n) * 1.5)
    open_ = close + np.random.randn(n) * 0.5
    volume = np.random.randint(1000, 10000, n).astype(float)
    high = np.maximum(high, np.maximum(open_, close))
    low = np.minimum(low, np.minimum(open_, close))
    funding_rate = np.random.uniform(-0.001, 0.001, n)
    return pd.DataFrame(
        {
            "open": open_,
            "high": high,
            "low": low,
            "close": close,
            "volume": volume,
            "funding_rate": funding_rate,
        },
        index=pd.date_range("2024-01-01", periods=n, freq="D"),
    )


class TestRegistry:
    def test_registered(self) -> None:
        assert "entropy-carry-mom" in list_strategies()

    def test_get_strategy(self) -> None:
        cls = get_strategy("entropy-carry-mom")
        assert cls is EntropyCarryMomStrategy


class TestEntropyCarryMomStrategy:
    def test_name(self) -> None:
        strategy = EntropyCarryMomStrategy()
        assert strategy.name == "entropy-carry-mom"

    def test_required_columns(self) -> None:
        strategy = EntropyCarryMomStrategy()
        assert "close" in strategy.required_columns
        assert "volume" in strategy.required_columns
        assert "funding_rate" in strategy.required_columns

    def test_config(self) -> None:
        strategy = EntropyCarryMomStrategy()
        assert isinstance(strategy.config, EntropyCarryMomConfig)

    def test_preprocess(self, sample_df: pd.DataFrame) -> None:
        strategy = EntropyCarryMomStrategy()
        result = strategy.preprocess(sample_df)
        assert len(result) == len(sample_df)

    def test_generate_signals(self, sample_df: pd.DataFrame) -> None:
        strategy = EntropyCarryMomStrategy()
        df = strategy.preprocess(sample_df)
        signals = strategy.generate_signals(df)
        assert len(signals.entries) == len(df)

    def test_run_pipeline(self, sample_df: pd.DataFrame) -> None:
        strategy = EntropyCarryMomStrategy()
        processed, signals = strategy.run(sample_df)
        assert len(processed) == len(sample_df)
        assert len(signals.entries) == len(sample_df)

    def test_from_params(self) -> None:
        strategy = EntropyCarryMomStrategy.from_params(entropy_window=40)
        assert isinstance(strategy, EntropyCarryMomStrategy)

    def test_recommended_config(self) -> None:
        config = EntropyCarryMomStrategy.recommended_config()
        assert isinstance(config, dict)
        assert "stop_loss_pct" in config

    def test_get_startup_info(self) -> None:
        strategy = EntropyCarryMomStrategy()
        info = strategy.get_startup_info()
        assert isinstance(info, dict)
        assert len(info) > 0

    def test_custom_config(self) -> None:
        config = EntropyCarryMomConfig(entropy_window=40)
        strategy = EntropyCarryMomStrategy(config=config)
        assert strategy._config.entropy_window == 40

    def test_params_property(self) -> None:
        strategy = EntropyCarryMomStrategy()
        params = strategy.params
        assert isinstance(params, dict)
        assert "entropy_window" in params

    def test_repr(self) -> None:
        strategy = EntropyCarryMomStrategy()
        assert "entropy-carry-mom" in strategy.name
        assert repr(strategy)
