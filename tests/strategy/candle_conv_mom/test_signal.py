"""Tests for candle-conv-mom signal generation."""
...
@pytest.fixture
def config() -> CandleConvMomConfig:
    return CandleConvMomConfig()
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: CandleConvMomConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: CandleConvMomConfig
    ) -> None:
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: CandleConvMomConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
