"""Tests for Volatility Ratio Trend signal generation."""
...
class TestShortMode:
    def test_disabled_no_shorts(self) -> None:
        config = VolRatioTrendConfig(short_mode=ShortMode.DISABLED)
        df = preprocess(_make_df(), config)
        signals = generate_signals(df, config)
        assert (signals.direction >= 0).all()

    def test_full_allows_shorts(self) -> None:
        config = VolRatioTrendConfig(short_mode=ShortMode.FULL)
        df = preprocess(_make_df(), config)
        signals = generate_signals(df, config)
