"""Tests for Kurtosis Carry signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: KurtosisCarryConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: KurtosisCarryConfig
    ) -> None:
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: KurtosisCarryConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
...
class TestShortMode:
    def test_disabled_no_shorts(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = KurtosisCarryConfig(short_mode=ShortMode.DISABLED)
    ...
    def test_hedge_only_respects_drawdown(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = KurtosisCarryConfig(short_mode=ShortMode.HEDGE_ONLY)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert signals.direction.dtype == int

    def test_full_allows_shorts(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = KurtosisCarryConfig(short_mode=ShortMode.FULL)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert signals.direction.dtype == int
