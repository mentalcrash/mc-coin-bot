"""Tests for Complexity-Filtered Trend signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: ComplexityTrendConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: ComplexityTrendConfig
    ) -> None:
...
class TestComplexityLogic:
    ...
    def test_strength_scales_with_conviction(
        self, preprocessed_df: pd.DataFrame, config: ComplexityTrendConfig
    ) -> None:
        """strength가 complexity conviction에 비례하는지 확인."""
        signals = generate_signals(preprocessed_df, config)
        # strength에 NaN이 없고, 유한한 값만 있어야 함
