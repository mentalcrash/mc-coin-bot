"""Tests for Triple-Channel Multi-Scale Trend signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: TriChannelTrendConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: TriChannelTrendConfig
    ) -> None:
...
class TestShortMode:
    ...
    def test_hedge_only_shorts_require_drawdown(self, sample_ohlcv_df: pd.DataFrame) -> None:
        ...
        config = TriChannelTrendConfig(short_mode=ShortMode.HEDGE_ONLY)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
...
class TestConsensusLogic:
    def test_unanimous_long(self, config: TriChannelTrendConfig) -> None:
    ...
    def test_entry_threshold_filters_weak_consensus(self, sample_ohlcv_df: pd.DataFrame) -> None:
        ...
        config_low = TriChannelTrendConfig(entry_threshold=0.0)
        config_high = TriChannelTrendConfig(entry_threshold=0.67)
    ...
    def test_nine_channels_consensus(self, config: TriChannelTrendConfig) -> None:
        """9개 sub-signal이 사용되는지 간접 확인: 전략별 채널 컬럼 필요."""
        np.random.seed(42)
        n = 200
        close = 100 + np.cumsum(np.random.randn(n) * 2)
        high = close + np.abs(np.random.randn(n) * 1.5)
        ...
        processed = preprocess(df, config)
        # 필요 채널 컬럼이 모두 있어야 generate_signals가 정상 작동
        signals = generate_signals(processed, config)
        assert len(signals.direction) == n
