"""Tests for Conviction Trend Composite signal generation."""
...
@pytest.fixture
def preprocessed_df(
    sample_ohlcv_df: pd.DataFrame, config: ConvictionTrendCompositeConfig
) -> pd.DataFrame:
...
class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: ConvictionTrendCompositeConfig
    ) -> None:
...
class TestShortModeVariants:
    def test_disabled_no_shorts(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = ConvictionTrendCompositeConfig(short_mode=ShortMode.DISABLED)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert (signals.direction >= 0).all()

    def test_full_allows_shorts(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = ConvictionTrendCompositeConfig(short_mode=ShortMode.FULL)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert signals.direction.dtype == int

    def test_hedge_only_mode(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = ConvictionTrendCompositeConfig(short_mode=ShortMode.HEDGE_ONLY)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert signals.direction.dtype == int
...
class TestRegimeAdaptation:
    ...
    def test_trending_more_aggressive(self, preprocessed_df: pd.DataFrame) -> None:
        ...
        config_default = ConvictionTrendCompositeConfig()
...
class TestConvictionTrendCompositeLogic:
    def test_uptrend_with_conviction_generates_long(self) -> None:
        """Clear uptrend + high conviction → long signal."""
        n = 300
        ...
        config = ConvictionTrendCompositeConfig(conviction_threshold=0.0)
        df = pd.DataFrame(
            {
                "open": close,
                "high": high,
        ...
        if (late != 0).any():
            # Should be mostly long in clear uptrend
            assert (late >= 0).all()

    def test_high_threshold_reduces_signals(self, sample_ohlcv_df: pd.DataFrame) -> None:
        """Higher conviction threshold should produce fewer signals."""
        config_low = ConvictionTrendCompositeConfig(conviction_threshold=0.0)
        config_high = ConvictionTrendCompositeConfig(conviction_threshold=1.0)
