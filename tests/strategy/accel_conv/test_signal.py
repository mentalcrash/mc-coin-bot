"""Tests for Acceleration-Conviction Momentum signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: AccelConvConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(self, preprocessed_df: pd.DataFrame, config: AccelConvConfig) -> None:
    ...
    def test_entries_exits_bool(
        self, preprocessed_df: pd.DataFrame, config: AccelConvConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert signals.entries.dtype == bool
        assert signals.exits.dtype == bool

    def test_direction_values(self, preprocessed_df: pd.DataFrame, config: AccelConvConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert set(signals.direction.unique()).issubset({-1, 0, 1})

    def test_same_length(self, preprocessed_df: pd.DataFrame, config: AccelConvConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
