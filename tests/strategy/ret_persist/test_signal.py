"""Tests for Return Persistence Score signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: RetPersistConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(self, preprocessed_df: pd.DataFrame, config: RetPersistConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert hasattr(signals, "entries")
        assert hasattr(signals, "direction")

    def test_entries_exits_bool(
        self, preprocessed_df: pd.DataFrame, config: RetPersistConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
