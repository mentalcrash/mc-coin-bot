"""Tests for FgEmaCycle signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: FgEmaCycleConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(self, preprocessed_df: pd.DataFrame, config: FgEmaCycleConfig) -> None:
    ...
    def test_entries_exits_bool(
        self, preprocessed_df: pd.DataFrame, config: FgEmaCycleConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: FgEmaCycleConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
