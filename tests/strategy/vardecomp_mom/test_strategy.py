"""Tests for Variance Decomposition Momentum strategy integration."""
...
class TestRegistry:
    def test_registered(self) -> None:
        assert "vardecomp-mom" in list_strategies()

    def test_get_strategy(self) -> None:
        cls = get_strategy("vardecomp-mom")
        assert cls is VardecompMomStrategy


class TestVardecompMomStrategy:
    def test_name(self) -> None:
        strategy = VardecompMomStrategy()
        assert strategy.name == "vardecomp-mom"
    ...
    def test_config(self) -> None:
        strategy = VardecompMomStrategy()
        assert isinstance(strategy.config, VardecompMomConfig)
    ...
    def test_from_params(self) -> None:
        strategy = VardecompMomStrategy.from_params(semivar_window=50)
        assert isinstance(strategy, VardecompMomStrategy)

    def test_recommended_config(self) -> None:
        config = VardecompMomStrategy.recommended_config()
    ...
    def test_repr(self) -> None:
        strategy = VardecompMomStrategy()
        assert "vardecomp-mom" in strategy.name
