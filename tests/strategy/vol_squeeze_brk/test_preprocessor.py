"""Tests for Vol Squeeze Breakout preprocessor."""
...
class TestPreprocess:
    ...
    def test_same_length(self, sample_ohlcv_df: pd.DataFrame, config: VolSqueezeBrkConfig) -> None:
        result = preprocess(sample_ohlcv_df, config)
        assert len(result) == len(sample_ohlcv_df)

    def test_immutability(self, sample_ohlcv_df: pd.DataFrame, config: VolSqueezeBrkConfig) -> None:
        original = sample_ohlcv_df.copy()
        preprocess(sample_ohlcv_df, config)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)

    def test_missing_columns(self, config: VolSqueezeBrkConfig) -> None:
