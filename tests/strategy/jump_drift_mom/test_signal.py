"""Tests for Jump Drift Momentum signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: JumpDriftMomConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(self, preprocessed_df: pd.DataFrame, config: JumpDriftMomConfig) -> None:
    ...
    def test_entries_exits_bool(
        self, preprocessed_df: pd.DataFrame, config: JumpDriftMomConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: JumpDriftMomConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
...
class TestJumpLogic:
    def test_lower_threshold_more_signals(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config_strict = JumpDriftMomConfig(jump_zscore_threshold=4.0)
        config_loose = JumpDriftMomConfig(jump_zscore_threshold=0.5)
