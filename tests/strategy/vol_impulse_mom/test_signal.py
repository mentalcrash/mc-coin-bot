"""Tests for vol-impulse-mom signal generation."""
...
@pytest.fixture
def config() -> VolImpulseMomConfig:
    return VolImpulseMomConfig()
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: VolImpulseMomConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: VolImpulseMomConfig
    ) -> None:
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: VolImpulseMomConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
