"""Tests for CCI Consensus Multi-Scale Trend preprocessor."""
...
class TestPreprocess:
    def test_output_columns(
        ...
        # CCI feature (1 indicator x 3 scales = 3 columns)
        for s in (config.scale_short, config.scale_mid, config.scale_long):
            assert f"cci_{s}" in result.columns
    ...
    def test_missing_columns(self, config: CciConsensusConfig) -> None:
    ...
    def test_custom_config(self, sample_ohlcv_df: pd.DataFrame) -> None:
        ...
        result = preprocess(sample_ohlcv_df, config)
        assert f"cci_{config.scale_short}" in result.columns
