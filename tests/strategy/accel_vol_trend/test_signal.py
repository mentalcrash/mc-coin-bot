"""Tests for Acceleration-Volatility Trend signal generation."""
...
@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: AccelVolTrendConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: AccelVolTrendConfig
    ) -> None:
    ...
    def test_same_length(self, preprocessed_df: pd.DataFrame, config: AccelVolTrendConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
...
class TestAccelVolLogic:
    ...
    def test_entries_exits_not_simultaneous(
        self, preprocessed_df: pd.DataFrame, config: AccelVolTrendConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
