"""Tests for EMA Multi-Cross signal generation."""

from __future__ import annotations

import numpy as np
import pandas as pd
import pytest

from src.strategy.ema_multi_cross.config import EmaMultiCrossConfig, ShortMode
from src.strategy.ema_multi_cross.preprocessor import preprocess
from src.strategy.ema_multi_cross.signal import generate_signals


@pytest.fixture
def config() -> EmaMultiCrossConfig:
    return EmaMultiCrossConfig()


@pytest.fixture
def sample_ohlcv_df() -> pd.DataFrame:
    np.random.seed(42)
    n = 300
    close = 100 + np.cumsum(np.random.randn(n) * 2)
    high = close + np.abs(np.random.randn(n) * 1.5)
    low = close - np.abs(np.random.randn(n) * 1.5)
    open_ = close + np.random.randn(n) * 0.5
    volume = np.random.randint(1000, 10000, n).astype(float)
    high = np.maximum(high, np.maximum(open_, close))
    low = np.minimum(low, np.minimum(open_, close))
    return pd.DataFrame(
        {"open": open_, "high": high, "low": low, "close": close, "volume": volume},
        index=pd.date_range("2024-01-01", periods=n, freq="1D"),
    )


@pytest.fixture
def preprocessed_df(sample_ohlcv_df: pd.DataFrame, config: EmaMultiCrossConfig) -> pd.DataFrame:
    return preprocess(sample_ohlcv_df, config)


class TestSignalStructure:
    def test_output_fields(
        self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert hasattr(signals, "entries")
        assert hasattr(signals, "exits")
        assert hasattr(signals, "direction")
        assert hasattr(signals, "strength")

    def test_entries_exits_bool(
        self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert signals.entries.dtype == bool
        assert signals.exits.dtype == bool

    def test_direction_values(
        self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert set(signals.direction.unique()).issubset({-1, 0, 1})

    def test_same_length(self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig) -> None:
        signals = generate_signals(preprocessed_df, config)
        n = len(preprocessed_df)
        assert len(signals.entries) == n
        assert len(signals.exits) == n
        assert len(signals.direction) == n
        assert len(signals.strength) == n

    def test_no_nan_in_strength(
        self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert not signals.strength.isna().any()


class TestShift1Rule:
    def test_first_bar_neutral(
        self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert signals.direction.iloc[0] == 0
        assert signals.strength.iloc[0] == 0.0


class TestConsensus:
    def test_min_votes_2_allows_majority(
        self, preprocessed_df: pd.DataFrame, config: EmaMultiCrossConfig
    ) -> None:
        signals = generate_signals(preprocessed_df, config)
        assert signals.direction.dtype == int

    def test_min_votes_3_stricter(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config_strict = EmaMultiCrossConfig(min_votes=3)
        df = preprocess(sample_ohlcv_df, config_strict)
        signals_strict = generate_signals(df, config_strict)

        config_loose = EmaMultiCrossConfig(min_votes=2)
        df2 = preprocess(sample_ohlcv_df, config_loose)
        signals_loose = generate_signals(df2, config_loose)

        # 만장일치(3)는 2/3 합의보다 거래 빈도가 같거나 낮아야 함
        strict_trades = (signals_strict.direction != 0).sum()
        loose_trades = (signals_loose.direction != 0).sum()
        assert strict_trades <= loose_trades


class TestShortModes:
    def test_disabled_no_shorts(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = EmaMultiCrossConfig(short_mode=ShortMode.DISABLED)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert (signals.direction >= 0).all()

    def test_full_allows_shorts(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = EmaMultiCrossConfig(short_mode=ShortMode.FULL)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert signals.direction.dtype == int

    def test_hedge_only_direction(self, sample_ohlcv_df: pd.DataFrame) -> None:
        config = EmaMultiCrossConfig(short_mode=ShortMode.HEDGE_ONLY)
        df = preprocess(sample_ohlcv_df, config)
        signals = generate_signals(df, config)
        assert set(signals.direction.unique()).issubset({-1, 0, 1})
