"""Tests for Vol-of-Vol Momentum strategy integration."""
...
class TestRegistry:
    def test_registered(self) -> None:
        assert "vov-mom" in list_strategies()

    def test_get_strategy(self) -> None:
        cls = get_strategy("vov-mom")
        assert cls is VovMomStrategy


class TestVovMomStrategy:
    def test_name(self) -> None:
        strategy = VovMomStrategy()
        assert strategy.name == "vov-mom"
    ...
    def test_config(self) -> None:
        strategy = VovMomStrategy()
        assert isinstance(strategy.config, VovMomConfig)
    ...
    def test_from_params(self) -> None:
        strategy = VovMomStrategy.from_params(gk_window=30)
        assert isinstance(strategy, VovMomStrategy)

    def test_recommended_config(self) -> None:
        config = VovMomStrategy.recommended_config()
    ...
    def test_repr(self) -> None:
        strategy = VovMomStrategy()
        assert "vov-mom" in strategy.name
