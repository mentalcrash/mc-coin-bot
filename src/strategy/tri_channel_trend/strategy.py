"""Triple-Channel Multi-Scale Trend 전략.

3종 채널(Donchian/Keltner/BB) x 3스케일(20/60/150) 앙상블 breakout으로 로버스트 추세 추종.
"""
...
class TriChannelTrendStrategy(BaseStrategy):
    """Triple-Channel Multi-Scale Trend 전략 구현.

    3종 채널 유형(Donchian/Keltner/BB) x 3스케일(20/60/150)의 9개 sub-signal
    consensus로 로버스트한 추세 방향을 도출한다.
    """

    def __init__(self, config: TriChannelTrendConfig | None = None) -> None:
        self._config = config or TriChannelTrendConfig()

    @property
    def name(self) -> str:
        return "tri-channel-trend"
    ...
    def from_params(cls, **params: Any) -> BaseStrategy:
        config = TriChannelTrendConfig(**params)
    ...
    def get_startup_info(self) -> dict[str, str]:
        return {
            ...
            "bb_std_dev": str(self._config.bb_std_dev),
