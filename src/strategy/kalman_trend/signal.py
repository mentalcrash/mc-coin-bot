"""Adaptive Kalman Trend Signal Generator.

칼만 필터 velocity로 trend direction 판별 후 시그널 생성.

Signal Formula:
    1. Shift(1) on kalman_velocity, vol_scalar
...
def generate_signals(
    ...
    """Adaptive Kalman Trend 시그널 생성.

    Args:
        df: 전처리된 DataFrame (preprocess() 출력)
        config: Kalman Trend 설정. None이면 기본 설정 사용.
    ...
    required_cols = {"kalman_velocity", "vol_scalar"}
    ...
    velocity_prev: pd.Series = df["kalman_velocity"].shift(1)  # type: ignore[assignment]
    vol_scalar_prev: pd.Series = df["vol_scalar"].shift(1)  # type: ignore[assignment]

    # 2. Direction classification based on velocity threshold
    #    velocity > threshold → LONG (+1)
    #    velocity < -threshold → SHORT (-1)
    #    |velocity| < threshold → FLAT (0)
    direction_raw = pd.Series(
        np.where(
            velocity_prev > config.vel_threshold,
            1,
            np.where(velocity_prev < -config.vel_threshold, -1, 0),
        ),
        index=df.index,
    )
