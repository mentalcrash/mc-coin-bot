"""VWAP-Channel Multi-Scale 시그널 생성.

3-scale VWAP channel breakout consensus 기반 시그널.

Shift(1) Rule: 모든 feature는 shift(1) 적용 후 시그널 계산.
close는 shift하지 않음: 현재 close vs 전봉 VWAP band 비교.
"""
...
def generate_signals(df: pd.DataFrame, config: VwapChannelConfig) -> StrategySignals:
    """VWAP-Channel Multi-Scale 시그널 생성.

    Signal Logic:
        1. 각 스케일에서 close vs prev_vwap_band breakout sub-signal 계산 (3개)
           - close > prev_vwap_upper -> +1
           - close < prev_vwap_lower -> -1
           - else -> 0
    ...
    """
    from src.strategy.vwap_channel_12h.config import ShortMode
    ...
    # close는 shift하지 않음: 현재 close vs 전봉 VWAP channel 비교
    close: pd.Series = df["close"]  # type: ignore[assignment]
    signal_components: list[pd.Series] = []

    # --- 3-Scale VWAP breakout sub-signals ---
    for s in scales:
        prev_upper = df[f"vwap_upper_{s}"].shift(1)
        prev_lower = df[f"vwap_lower_{s}"].shift(1)
        signal_i = pd.Series(
            np.where(
                close > prev_upper,
                1.0,
