"""Wavelet-Channel 8H 시그널 생성.

DWT denoised 3종 채널(Donchian/Keltner/BB) x 3스케일(22/66/132) breakout consensus 시그널.

Shift(1) Rule: 모든 feature는 shift(1) 적용 후 시그널 계산.
IMPORTANT: raw close (shift 없음) vs prev channel (shift(1)) 비교.
"""
...
def generate_signals(df: pd.DataFrame, config: WaveletChannel8hConfig) -> StrategySignals:
    """Wavelet-Channel 8H 시그널 생성.

    Signal Logic:
        1. 각 (채널, 스케일) 조합에 대해 breakout sub-signal 계산 (총 9개)
           - Donchian: raw_close > prev_upper -> +1, raw_close < prev_lower -> -1
           - Keltner: raw_close > prev_upper -> +1, raw_close < prev_lower -> -1
           - BB: raw_close > prev_upper -> +1, raw_close < prev_lower -> -1
    ...
    """
    from src.strategy.wavelet_channel_8h.config import ShortMode
    ...
    # raw close는 shift하지 않음: 현재 close vs 전봉 channel 비교
