"""Vol Structure Regime Signal Generator.

이 모듈은 전처리된 데이터에서 매매 시그널을 생성합니다.
VectorBT 및 QuantStats와 호환되는 표준 출력을 제공합니다.

Signal Formula:
    1. vol_ratio, norm_momentum에서 regime 분류 (shift(1) 적용)
    2. Expansion → follow momentum direction (±1)
    3. Contraction → flat (0)
    4. Neutral → moderate signal (±0.5)
...
def generate_signals(
    df: pd.DataFrame,
    config: VolStructureConfig | None = None,
) -> StrategySignals:
    """Vol Structure Regime 시그널 생성.
    ...
    Args:
        df: 전처리된 DataFrame (preprocess() 출력)
            필수 컬럼: vol_ratio, norm_momentum, vol_scalar
        config: Vol Structure Regime 설정. None이면 기본 설정 사용.
    ...
    # 입력 검증
    required_cols = {"vol_ratio", "norm_momentum", "vol_scalar"}
    ...
    norm_mom_prev: pd.Series = df["norm_momentum"].shift(1)  # type: ignore[assignment]
    vol_scalar_prev: pd.Series = df["vol_scalar"].shift(1)  # type: ignore[assignment]

    # 2. Regime classification
    expansion = (vol_ratio_prev > config.expansion_vol_ratio) & (
        norm_mom_prev.abs() > config.expansion_mom_threshold
    )
    contraction = (vol_ratio_prev < config.contraction_vol_ratio) & (
        norm_mom_prev.abs() < config.contraction_mom_threshold
    )

    # 3. Direction:
    #    Expansion → strong trend, follow momentum direction (±1)
    #    Contraction → range-bound, FLAT (0)
    #    Neutral → moderate signal (±0.5)
    direction_raw = pd.Series(
        np.where(
            expansion,
            np.sign(norm_mom_prev),
            np.where(contraction, 0, np.sign(norm_mom_prev) * 0.5),
        ),
        index=df.index,
    )
