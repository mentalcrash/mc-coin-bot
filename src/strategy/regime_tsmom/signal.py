...
레짐 확률에 따라 vol_target과 leverage scale을 적응적으로 조절합니다.
...
Signal Formula:
    ...
    4. leverage_scale = p_trending * tls + p_ranging * rls + p_volatile * vls
    5. strength = direction * vol_scalar * leverage_scale  (shifted by 1)
...
def generate_signals(
    ...
    Args:
        df: 전처리된 DataFrame (preprocess() 출력)
            필수 컬럼: vw_momentum, vol_scalar, realized_vol,
    ...
    # 2. Probability-weighted adaptive vol_target
    ...
    # 4. Probability-weighted leverage scale
    leverage_scale: pd.Series = (  # type: ignore[assignment]
        p_trending * config.trending_leverage_scale
        + p_ranging * config.ranging_leverage_scale
        + p_volatile * config.volatile_leverage_scale
    )

    # 5. Raw strength = direction * vol_scalar * leverage_scale
    raw_strength = momentum_direction * vol_scalar * leverage_scale

    # 6. Shift(1): 미래 참조 편향 방지
    ...
    if len(valid_strength) > 0:
        ...
        if len(valid_regime) > 0:
            ...
            logger.info("🏷️ Regime Distribution | %s", regime_str)

        avg_vol_target = adaptive_vol_target.dropna().mean()
        avg_leverage = leverage_scale.dropna().mean()
        logger.info(
            "📊 Adaptive Params | Avg Vol Target: %.2f, Avg Lev Scale: %.2f",
            avg_vol_target,
            avg_leverage,
        )
