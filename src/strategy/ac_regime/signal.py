"""Autocorrelation Regime-Adaptive Signal Generator.

AC 부호로 regime 분류 후 momentum/contrarian 자동 전환.

Signal Formula:
    1. Shift(1) on ac_rho, sig_bound, mom_direction, vol_scalar
    2. Trending: rho > 0 AND |rho| > sig_bound → direction = mom_direction
    3. Mean-reverting: rho < 0 AND |rho| > sig_bound → direction = -mom_direction
    4. Neutral: else → direction = 0
...
def generate_signals(
    df: pd.DataFrame,
    config: ACRegimeConfig | None = None,
) -> StrategySignals:
    """Autocorrelation Regime-Adaptive 시그널 생성.

    Args:
        df: 전처리된 DataFrame (preprocess() 출력)
        config: AC Regime 설정. None이면 기본 설정 사용.
    ...
    required_cols = {"ac_rho", "sig_bound", "mom_direction", "vol_scalar"}
    ...
    rho_prev: pd.Series = df["ac_rho"].shift(1)  # type: ignore[assignment]
    sig_bound_prev: pd.Series = df["sig_bound"].shift(1)  # type: ignore[assignment]
    ...
    abs_rho = rho_prev.abs()
    significant = abs_rho > sig_bound_prev

    trending = (rho_prev > 0) & significant
    mean_reverting = (rho_prev < 0) & significant

    # 3. Direction:
    #    Trending → follow momentum direction
