"""Carry-Conditional Momentum 전략 설정.

가격 모멘텀과 funding rate LEVEL의 agreement로 모멘텀 지속 가능성 예측.
mom(+) AND FR(+) = bullish consensus → confident long. 불일치 시 position 축소.
"""
...
class CarryCondMomConfig(BaseModel):
    """Carry-Conditional Momentum 전략 설정.
    ...
    Attributes:
        ...
        agreement_boost: Mom+FR 동의 시 strength 부스트 비율.
        disagreement_penalty: Mom+FR 불일치 시 strength 감쇄 비율.
    ...
    agreement_boost: float = Field(default=1.2, ge=1.0, le=2.0)
    disagreement_penalty: float = Field(default=0.3, ge=0.0, le=1.0)
