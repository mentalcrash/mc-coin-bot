"""DEX Activity Momentum Signal Generation.

Direction logic:
    - dex_roc_short > 0 AND dex_roc_long > 0 → +1 (DEX activity expanding = risk-on)
    - dex_roc_short < 0 AND dex_roc_long < 0 → -1 (DEX activity contracting = risk-off)
...
def generate_signals(
    ...
    # DEX volume ROC scores
    if "dex_roc_short" in df.columns and "dex_roc_long" in df.columns:
        roc_short: pd.Series = df["dex_roc_short"].fillna(0.0)  # type: ignore[assignment]
        roc_long: pd.Series = df["dex_roc_long"].fillna(0.0)  # type: ignore[assignment]
    ...
    logger.info(
        "Dex-Mom signals | Total: {}, Long: {} ({:.1%}), Short: {} ({:.1%})",
        n_total,
        n_long,
