"""CTREND-X 시그널 생성 (Rolling GradientBoosting Prediction).

CTREND의 ElasticNet을 GradientBoostingRegressor로 교체하여
비선형 feature 상호작용을 캡처.
...
def generate_signals(
    df: pd.DataFrame,
    config: CTRENDXConfig | None = None,
    *,
    predict_last_only: bool = False,
) -> StrategySignals:
    """CTREND-X 시그널 생성 (Rolling GradientBoosting).

    Args:
        df: preprocess() 출력 DataFrame.
        config: 전략 설정. None이면 기본 설정.
        predict_last_only: True면 마지막 시그널만 계산 (EDA incremental 최적화).
