"""Fragility-Aware Momentum 전처리 모듈.

OHLCV 데이터에서 GK vol, VoV, GK vol percentile, 모멘텀 feature를 계산한다.
...
def preprocess(df: pd.DataFrame, config: FragilityMomConfig) -> pd.DataFrame:
    """Fragility-Aware Momentum feature 계산.
    ...
    Calculated columns:
        ...
        - gk_vol_pct: GK vol percentile rank (0~1)
        - price_mom: 가격 모멘텀
    ...
    # --- GK Vol Percentile Rank ---
    df["gk_vol_pct"] = vol_percentile_rank(gk_vol, window=config.gk_vol_percentile_window)
