"""Bronze/Silver storage for macro economic data (FRED + yfinance).

Bronze: append-only parquet (zstd compression)
Silver: dedup → sort → UTC enforce → numeric coercion → save

Path convention: data/{bronze,silver}/macro/{source}/{name}.parquet
...
class MacroBronzeStorage:
    """Bronze 계층 저장소 — macro 데이터.

    변형 없이 원본 그대로 저장합니다.

    Example:
        >>> storage = MacroBronzeStorage()
        >>> path = storage.save(df, "fred", "dxy")
    """
    ...
    def save(self, df: pd.DataFrame, source: str, name: str) -> Path:
        ...
        path = self.settings.get_macro_bronze_path(source, name)
        ...
        try:
            ...
            raise StorageError(
                f"Failed to save Bronze macro to {path}",
    ...
    def append(self, df: pd.DataFrame, source: str, name: str, dedup_col: str = "date") -> Path:
        ...
        try:
            ...
            raise StorageError(
                f"Failed to append Bronze macro to {path}",
                context={"path": str(path), "error": str(e)},
            ) from e
        else:
            return path
    ...
    def load(self, source: str, name: str) -> pd.DataFrame:
        ...
        if not path.exists():
            raise StorageError(
                f"Bronze macro file not found: {path}",
        ...
        try:
            ...
            raise StorageError(
                f"Failed to load Bronze macro from {path}",
                context={"path": str(path), "error": str(e)},
            ) from e
        else:
            return df
    ...
    def exists(self, source: str, name: str) -> bool:
        """Bronze 파일 존재 여부."""
        return self.settings.get_macro_bronze_path(source, name).exists()
...
class MacroSilverProcessor:
    """Silver 계층 프로세서 — macro 데이터.

    Bronze 데이터를 정제합니다: 중복 제거 → 정렬 → UTC enforce → numeric coercion → 저장.
    FRED value=None → Silver에서 NaN (forward-fill은 전략에서 결정).

    Example:
        >>> processor = MacroSilverProcessor()
        >>> path = processor.process("fred", "dxy")
    """

    def __init__(
        self,
        settings: IngestionSettings | None = None,
        bronze_storage: MacroBronzeStorage | None = None,
    ) -> None:
        self.settings = settings or get_settings()
        self.bronze_storage = bronze_storage or MacroBronzeStorage(self.settings)
    ...
    def process(
        ...
        """
        logger.info(
            "Processing macro Bronze → Silver for {}/{}",
            source,
            name,
        )
        ...
        _text_cols = {"source", "name", "series_id", "ticker", date_col}
        ...
        silver_path = self.settings.get_macro_silver_path(source, name)
        ...
        try:
            ...
            raise StorageError(
                f"Failed to save Silver macro to {silver_path}",
    ...
    def load(self, source: str, name: str) -> pd.DataFrame:
        ...
        Returns:
            pandas DataFrame
        """
        path = self.settings.get_macro_silver_path(source, name)
        if not path.exists():
            raise StorageError(
                f"Silver macro file not found: {path}",
                context={"path": str(path), "source": source, "name": name},
            )
        try:
            df = pd.read_parquet(path)
            ...
            raise StorageError(
                f"Failed to load Silver macro from {path}",
                context={"path": str(path), "error": str(e)},
            ) from e
        else:
            return df
    ...
    def exists(self, source: str, name: str) -> bool:
        """Silver 파일 존재 여부."""
        return self.settings.get_macro_silver_path(source, name).exists()
    ...
    def _validate_data(self, df: pd.DataFrame, source: str, name: str) -> None:
        ...
        if errors:
            raise DataIntegrityError(
                f"Macro data validation failed for {source}/{name}",
