"""Rate-limited async HTTP client for Coinalyze API v1.

AsyncCoinalyzeClient: httpx + rate limiter + retry (exponential backoff)

Rules Applied:
    - #23 Exception Handling: Domain-driven hierarchy
    - #19 Git Security: No secrets in code (api_key via parameter)
"""
...
DERIV_EXT_RATE_LIMITS: dict[str, int] = {
    "coinalyze": 40,  # Free/Hobbyist tier: 40 req/min
    "hyperliquid": 60,  # Public API, no auth required
}
...
class AsyncCoinalyzeClient:
    """Rate-limited async HTTP client for Coinalyze API.

    Uses httpx.AsyncClient with rate limiting and exponential backoff retry.
    Requires api_key for authentication via header.

    Example:
        >>> async with AsyncCoinalyzeClient("coinalyze", api_key="...") as client:
        ...     resp = await client.get("open-interest-history", params=...)
    """
    ...
    def __init__(
        ...
        *,
        ...
        rpm = DERIV_EXT_RATE_LIMITS.get(source, 40)
        self._rate_limiter = _RateLimiter(rpm)

    async def __aenter__(self) -> AsyncCoinalyzeClient:
    ...
    async def get(self, endpoint: str, **kwargs: Any) -> httpx.Response:
        ...
        if self._client is None:
            msg = "Client not initialized. Use 'async with AsyncCoinalyzeClient(...)' context manager."
...
class AsyncHyperliquidClient:
    """Rate-limited async HTTP client for Hyperliquid API.

    Hyperliquid는 인증 불필요, POST-only API입니다.
    읽기 전용 요청이므로 retry가 안전합니다.
    ...
    def __init__(
        ...
        *,
        ...
        self._source = "hyperliquid"
        self._max_retries = max_retries
        self._backoff_base = backoff_base
        self._timeout = timeout
        self._client: httpx.AsyncClient | None = None

        rpm = DERIV_EXT_RATE_LIMITS.get("hyperliquid", 60)
        self._rate_limiter = _RateLimiter(rpm)

    async def __aenter__(self) -> AsyncHyperliquidClient:
        """Enter async context: create httpx client."""
        self._client = httpx.AsyncClient(
    ...
    async def post(self, endpoint: str, *, json: dict[str, Any] | None = None) -> httpx.Response:
        ...
        if self._client is None:
            msg = (
                "Client not initialized. Use 'async with AsyncHyperliquidClient(...)' "
                "context manager."
            )
            raise RuntimeError(msg)
