"""Bronze/Silver storage for Deribit Options data.

Bronze: append-only parquet (zstd compression)
Silver: dedup → sort → UTC enforce → numeric coercion → save

Path convention: data/{bronze,silver}/options/{source}/{name}.parquet
...
class OptionsBronzeStorage:
    """Bronze 계층 저장소 — Options 데이터.

    변형 없이 원본 그대로 저장합니다.

    Example:
        >>> storage = OptionsBronzeStorage()
        >>> path = storage.save(df, "deribit", "btc_dvol")
    """
    ...
    def save(self, df: pd.DataFrame, source: str, name: str) -> Path:
        ...
        path = self.settings.get_options_bronze_path(source, name)
        path.parent.mkdir(parents=True, exist_ok=True)

        try:
            df.to_parquet(path, compression="zstd", index=True)
            logger.info(
                "Bronze options saved: {}",
                path,
                extra={"path": str(path), "source": source, "name": name, "rows": len(df)},
            )
        except Exception as e:
            raise StorageError(
                f"Failed to save Bronze options to {path}",
    ...
    def append(self, df: pd.DataFrame, source: str, name: str, dedup_col: str = "date") -> Path:
        ...
        try:
            ...
            raise StorageError(
                f"Failed to append Bronze options to {path}",
                context={"path": str(path), "error": str(e)},
            ) from e
        else:
            return path
    ...
    def load(self, source: str, name: str) -> pd.DataFrame:
        ...
        if not path.exists():
            raise StorageError(
                f"Bronze options file not found: {path}",
                context={"path": str(path), "source": source, "name": name},
            )
        try:
            df = pd.read_parquet(path)
            logger.debug(
                "Bronze options loaded: {}",
                path,
                extra={"path": str(path), "rows": len(df)},
            )
        except Exception as e:
            raise StorageError(
                f"Failed to load Bronze options from {path}",
                context={"path": str(path), "error": str(e)},
            ) from e
        else:
            return df
    ...
    def exists(self, source: str, name: str) -> bool:
        """Bronze 파일 존재 여부."""
        return self.settings.get_options_bronze_path(source, name).exists()
...
class OptionsSilverProcessor:
    """Silver 계층 프로세서 — Options 데이터.

    Bronze 데이터를 정제합니다: 중복 제거 → 정렬 → UTC enforce → numeric coercion → 저장.

    Example:
        >>> processor = OptionsSilverProcessor()
        >>> path = processor.process("deribit", "btc_dvol")
    """

    def __init__(
        self,
        settings: IngestionSettings | None = None,
        bronze_storage: OptionsBronzeStorage | None = None,
    ) -> None:
        self.settings = settings or get_settings()
        self.bronze_storage = bronze_storage or OptionsBronzeStorage(self.settings)
    ...
    def process(
        ...
        """
        logger.info(
            "Processing options Bronze → Silver for {}/{}",
            source,
            name,
        )
        ...
        _text_cols = {"source", "currency", "near_expiry", "far_expiry", "expiry", date_col}
        ...
        silver_path = self.settings.get_options_silver_path(source, name)
        silver_path.parent.mkdir(parents=True, exist_ok=True)

        try:
            df.to_parquet(silver_path, compression="zstd", index=True)
            logger.info(
                "Silver options saved: {}",
                silver_path,
                extra={
            ...
            raise StorageError(
                f"Failed to save Silver options to {silver_path}",
    ...
    def load(self, source: str, name: str) -> pd.DataFrame:
        ...
        Returns:
            pandas DataFrame
        """
        path = self.settings.get_options_silver_path(source, name)
        if not path.exists():
            raise StorageError(
                f"Silver options file not found: {path}",
                context={"path": str(path), "source": source, "name": name},
            )
        try:
            df = pd.read_parquet(path)
            logger.debug(
                "Silver options loaded: {}",
                path,
                extra={"path": str(path), "rows": len(df)},
            )
        except Exception as e:
            raise StorageError(
                f"Failed to load Silver options from {path}",
                context={"path": str(path), "error": str(e)},
            ) from e
        else:
            return df
    ...
    def exists(self, source: str, name: str) -> bool:
        """Silver 파일 존재 여부."""
        return self.settings.get_options_silver_path(source, name).exists()
    ...
    def _validate_data(self, df: pd.DataFrame, source: str, name: str) -> None:
        ...
        if errors:
            raise DataIntegrityError(
                f"Options data validation failed for {source}/{name}",
