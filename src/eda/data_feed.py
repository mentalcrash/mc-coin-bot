...
AggregatingDataFeed: 1m 데이터를 CandleAggregator로 집계하여 TF bar를 생성합니다.
...
class HistoricalDataFeed:
    ...
    def _validate_bar(
        ...
        """OHLCV bar 데이터 품질 검증 (M-004)."""
        return _validate_bar(o, h, lo, c, v, symbol, ts)


class AggregatingDataFeed:
    ...
    async def start(self, bus: EventBus) -> None:
        ...
        logger.info(
            "AggregatingDataFeed finished: {} bars emitted (target_tf={})",
            self._bars_emitted,
            self._target_tf,
        )
