...
mc-bot (:8000/metrics)
    -> Prometheus (scrape 10s)
        -> Grafana (6 primary dashboards)
        -> Alertmanager (alerts.yml, 38 rules)
...
- **auto\_init\_detectors() 호출 경로**: LiveRunner warmup 완료 후
  `lifecycle.auto_init_detectors(strategy_id, returns)`를 호출합니다.
  daily\_returns가 없으면 초기화 스킵.
...
`monitoring/grafana/dashboards/` 에 JSON으로 버전 관리 (6개 primary).
