...
## Phase 2: 통합 백테스트 결과 (VectorBT `cash_sharing`)

> **구현 완료:** 2026-02-06

이전 분석은 각 에셋의 독립 백테스트 수익률을 EW 평균한 결과였음.
Phase 2에서는 VectorBT `cash_sharing=True`로 **실제 자본 공유 포트폴리오**를 구현.

### 통합 vs 독립 비교

| 지표 | 통합 (cash_sharing) | 개별 평균 | 차이 |
|------|:---:|:---:|:---:|
| **CAGR** | +57.95% | +36.05% | +61% |
| **Sharpe** | 1.57 | 1.08 | +45% |
...
### EW 평균 방식 vs cash_sharing의 차이

| 방식 | Sharpe | 설명 |
|------|:---:|------|
| EW 평균 (스윕) | 2.41 | 각 에셋 독립 시뮬레이션 후 수익률 산술 평균 |
| cash_sharing (통합) | 1.57 | 실제 자본 공유, 거래비용/슬리피지/자본 제약 반영 |

**Sharpe 차이(2.41 → 1.57)의 원인:**
1. **거래비용 실제 반영**: 8에셋 동시 리밸런싱 시 비용 증가
2. **자본 제약**: 공유 자본이므로 한 에셋의 큰 포지션이 다른 에셋의 자본을 제약
3. **동시 드로다운**: 독립 평균에서는 상쇄되지만, 자본 공유에서는 실제 손실
4. **시점 차이**: 실제 체결 가격(next-open) vs 이론 close 가격

> **방향성은 일치**: 두 방식 모두 분산 효과(Sharpe 상승, MDD 감소)를 확인.
> 통합 결과가 더 보수적이고 현실적.

### 심볼별 성과 (통합 백테스트 내)

| Symbol | CAGR | Sharpe | MDD | Contribution |
|--------|:---:|:---:|:---:|:---:|
| BTC/USDT | +51.8% | 0.84 | -76.6% | +46.0% |
| ETH/USDT | +68.2% | 0.82 | -79.3% | +65.3% |
| BNB/USDT | +99.3% | 1.15 | -70.9% | +79.1% |
| SOL/USDT | +87.6% | 0.77 | -96.3% | +95.1% |
| DOGE/USDT | +98.2% | 0.48 | -92.3% | +134.8% |
| LINK/USDT | +37.8% | 0.35 | -90.2% | +67.7% |
| ADA/USDT | +47.9% | 0.47 | -91.8% | +67.9% |
| AVAX/USDT | +21.5% | 0.19 | -93.5% | +61.4% |

> 개별 심볼 MDD가 70~96%로 극단적이지만, 포트폴리오 MDD는 19.43%로 분산 효과가 극적.
