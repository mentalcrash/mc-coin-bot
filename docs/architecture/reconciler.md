...
Spot 운용에서는 One-Way Mode로 동작합니다.
`parse_exchange_positions()`가 `{symbol: (size, Direction)}` 형태로 거래소 포지션을 파싱하며,
PM 내부 포지션과 raw symbol 기준으로 직접 비교합니다.
...
| `parse_spot_balances()` | Spot 잔고 파싱 | `{symbol: free_amount}` |
